...
When in doubt, run the nine check scripts — `npx tsx dev/check-catan-board.ts`, `check-catan-placement.ts`, `check-catan-roll.ts`, `check-catan-build.ts`, `check-catan-trade.ts`, `check-catan-robber.ts`, `check-catan-ports.ts`, `check-catan-dev.ts`, `check-catan-longest-road.ts` — after edits. The edge function is only validated at deploy time (`npm run edge`).
