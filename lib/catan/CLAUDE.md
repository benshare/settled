...
- `gameContext.tsx` — React context that loads the per-game `games` row + `game_states` row and subscribes to realtime. Use `useGame()` in any subtree under `<GameProvider>`. Both rows are re-fetched and both channels re-subscribed on every app foreground (`useAppForeground`), because realtime events emitted while the app was backgrounded are never delivered — see `lib/stores/CLAUDE.md`.
