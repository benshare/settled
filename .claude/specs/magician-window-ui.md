...
2. **The window asks one question instead of two.** It used to be a grid of
   ...
   roll cards centered on the number that actually came up, each showing what
...
- **The arc.** One small card per number in reach, in numeric order, fanned and
  lifted like the hand's `CardFan` — the rolled number sits at the center, the
  arc's apex, and the rest spread away from it. Each card is the number over
  the cards that number would pay this player, and under that its price. The
  center card is labeled as the roll rather than priced; tapping it clears a
  selection (the same outcome as **Keep roll**). The arc scrolls horizontally
  and opens centered on the roll, since eleven cards don't fit a phone.
...
- `lib/catan/MagicianPickOverlay.tsx` — the redesign, plus the roll-arc fan.
